#include "WarehouseLinksBothTime.h"

WarehouseLinksBothTime::~WarehouseLinksBothTime(){
  delete whGraph ;
  whGraph = 0 ;
...
void WarehouseLinksBothTime::SimulateEpoch(bool train){
  size_t teamSize ;
  if (train)
    teamSize = 2*nPop ;
  else
...
void WarehouseLinksBothTime::SimulateEpoch(vector<size_t> memberIDs){
  double maxEval = 0.0 ;
  vector<size_t> travelStats ;
...
void WarehouseLinksBothTime::InitialiseMATeam(){
...
void WarehouseLinksBothTime::QueryMATeam(vector<size_t> memberIDs, vector<double> &a, vector<size_t> &s){
  vector<Edge *> e = whGraph->GetEdges() ;
  ...
  for (size_t i = 0; i < nAgents; i++){
    ...
    input(2) = eTime[whAgents[i]->vID + e.size()] ;
...
void WarehouseLinksBothTime::GetJointState(vector<Edge *> e, vector<size_t> &s, vector<double> &eTime){
