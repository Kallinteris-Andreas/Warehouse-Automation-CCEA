#include "WarehouseCentralisedBothTime.h"

WarehouseCentralisedBothTime::~WarehouseCentralisedBothTime(void){
  delete whGraph ;
  whGraph = 0 ;
...
void WarehouseCentralisedBothTime::SimulateEpoch(bool train){
  size_t teamSize ;
  if (train)
    teamSize = 2*nPop ;
  else
...
void WarehouseCentralisedBothTime::SimulateEpoch(vector<size_t> memberIDs){
  double maxEval = 0.0 ;
  vector<size_t> travelStats ;
...
void WarehouseCentralisedBothTime::InitialiseMATeam(){
  ...
  size_t nIn = nOut*3 ; // NN input is current #AGVs on all edges and average time to arrival at next intersection and time to arrival at next intersection
//  size_t nHid = 16 ; // fixed to compare against link agent formulation
  size_t nHid = 4*nIn/3 ; // control for relative representational capacity
  Agent * neAgent ;
...
void WarehouseCentralisedBothTime::QueryMATeam(vector<size_t> memberIDs, vector<double> &a, vector<size_t> &s){
  vector<Edge *> e = whGraph->GetEdges() ;
